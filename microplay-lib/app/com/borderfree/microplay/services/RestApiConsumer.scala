      ...
      response: WSResponse =>
        ...
        response.status match {
          case status if ValidHttpResponses.contains(status) =>
            logger.debug(s"for $httpMethod to endpoint $apiMethodUriSuffix - received api response within $callDuration millis.${if(shouldTraceResponseBody(apiMethodUriSuffix))" response: "+response.body}")
